...
// 기본형 변수 : 원시형(기본형)으로 생성된 변수 - 값을 저장하기 위한 변수 ex) int num = 100; / num /
// 참조형 변수 : 참조형으로 생성된 변수 - 특정 대상의 메모리 주소값을 저장하기 위한 변수 ex) String name = "홍길동"; / name /
// => heap영역에 생성된 배열 또는 객체의 메모리 주소를 저장하여 접근하기 위해 사용
// => 비교 연산자를 사용하여 참조변수를 비교할 경우 참조변수의 저장된 메모리 주소 비교
// => 참조변수에 배열 또는 객체를 저장하지 않을 경우 초기값으로 null 저장
// null : 아무것도 없음을 표현하기 위한 키워드

// Java 프로그램에서 사용하는 메모리 영역
// => 메소드 영역(Method Area), 힙 영역(Heap Area), 스택 영역(Stack Area)
// 1. 메소드 영역 : 클래스 파일(XXX.class)의 자바 자료형(클래스, 인터페이스, 나열형)을 읽어 저장하는 영역
// 2. 힙 영역 : 배열 또는 객체를 저장하기 위한 영역 - new 연산자 이용
// => 메모리에 이름 설정이 불가능하다 : 참조변수에 객체(배열)의 메모리 주소(HashCode)를 저장하여 접근
...
public class ArrayApp {
	public static void main(String[] args) {
		...
		// => 배열의 요소는에는 요소에는 기본값(숫자형 : 0, 논리형 : false, 참조형 : null)이 자동 저장
		
		int[] num = new int[3]; // 정수값 3개를 저장할 수 있는 배열 'num' 생성
		...
		int index=0;
		// 배열 요소를 구분하는 첨자는 변수또는 연산식 사용하여 표현 가능
		num[index]=10;
		num[index+1]=20;
		num[index+2]=30;
		...
		System.out.println("30~60 범위의 갯수 : " + count);
		System.out.println("==================================================================");
