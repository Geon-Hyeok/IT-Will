...
/* => 현실에 존재하는 사물 또는 관념을 클래스라는 자료형으로 표현하여 
		프로그램에 동작될 수 있도록 객체로 생성 */
...
// 			 ...
// 		}
// => 모든 메소드는 필드를 사용한 명령으로 필요한 기능을 제공되도록 작성
// => 클래스 내부에 선언된 모든 메소드는 필드 접근 가능

// 클래스를 작성할 때 필드와 메소드에는 접근 제한자를 사용하여 접근 허용 설정 가능
...
// 데이터 은닉화(Data Hiding) : 값을숨겨 보호하기 위한 기능
...
public class Car {
	...
	/*
	 * 생성자(Constructor) : 생성자를 선언하지 않으면 매개변수가 없는 기본 생성자(Default Constructor)가 자동으로
	 * 생성되어 제공
	 */
	...
	public void stopEngine() { // 시동 오프(Off)
		...
		if (currentSpeed != 0) { // 자동차가 멈춰있지 않은 상태인 경우
			/*
			 * currentSpeed=0; System.out.println(modelName + "의 자동차가 멈췄습니다.");
			 */
	...
	// Getter 메소드 : 클래스에 외부에서 필드값을 사용할수 있도록 필드값을 반환하는 메소드
	...
	// 단축키 : [Alt] + [Shift] + [S] : 팝업메뉴 - [R] >> Generate
