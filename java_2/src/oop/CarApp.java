...
// => 참조변수에 저장된 객체는 . 연산자를 사용하여 필드 또는 메소드를 이용해 프로그램 작성 

// Car 클래스를 객체로 생성하여 작성된 프로그램
public class CarApp {
	public static void main(String[] args) {
		// Car 클래스를 객체를 생상하여 참조변수에 저장
		// => 하나의 클래스로 서로 다른 객체를 여러개 생성 가능
		// => 클래스는 객체를 생성하기 위한 틀(Template)
		// => 객체를 생성하면 객체의 필드에서는 기본값(숫자형 : 0, 논리형 : false, 참조형 : null)이 초기값으로 자동저장
		Car carOne = new Car();
		Car carTwo = new Car();                
		...
		carOne.modelName = "싼타페"; // 객체의 필드에 값 저장
//		carOne.engineStatus = false; // 입력하려고 했던 값과 객체의 기본값이 같기에 생략 가능
