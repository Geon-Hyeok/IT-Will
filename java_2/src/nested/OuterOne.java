...
// 중첩 클래스(Nested Class) : 클래스(OuterClass) 내부에 다른 클래스(InnerClass)를 선언
...
public class OuterOne {
	...
	public void outerDisplay() {
		...
		// 외부클래스는 객체 내부클래스를 객체를 생성하여 접근제한자에
		// 상관 없이 필드 또는 메소드 참조 가능
		// InnerOne innerOne=new InnerOne();
		// System.out.println("innerNum = " + innerOne.innerNum);
		// innerOne.innerDisplay();
	}

	// 객체 내부 클래스 (Instance InnerClass)=> 컴파일 결과를 [외부클래스$내부클래스.class] 파일로 제공
	...
	public class InnerOne {
		...
		public void innerDisplay() {
			...
			// 객체 내부 클래스에서는 외부클래스의 필드 또는 메소드를 접근제한자에
			// 상관없이 직접적인 참조 가능
			System.out.println("outerNum = " + outerNum);
			outerDisplay();
		}
