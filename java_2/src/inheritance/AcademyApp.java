...
// 학원이상의 자원(학생, 강사, 직원) 관리 프로그램

public class AcademyApp {
	public static void main(String[] args) {
		/*
		// 학생 정보(AcademyStudent 객체)를 저장하기 위한 요소들이 존재하는 배열 생성
		...
		// 직원 정보(AcademyStaff 객체)를 저장하기 위한 요소들이 존재하는 배열 생성
		AcademyStaff[] staffs = new AcademyStaff[100];
		*/
		
		// 사람 정보(AcademyPerson 객체)를 저장하기 위한 요소들이 존재하는 배열 생성
		// => 배열 요소에 학생정보, 강사정보, 직원정보 저장 가능
		// 부모클래스의 참조변수에는 자식클래스의 생성자로 객체를 생성하여 저장 가능
		AcademyPerson[] persons = new AcademyPerson[5];
		
		// 자식 클래스의 생성자로 객체를 생성하면 부모클래스의 객체를먼저 생성한 후
		// 자식클래스의 객체를 생성
		// => 배열 요소에는 부모클래스의 객체가 저장되어 부모클래스의 메소드 호출
		// => 객체의 형변환을 이용하면 배열 요소를 사용하여 자식클래스의 객체를
		// 일시적으로 저장하여 자식클래스의 메소드 호출 가능
		...
		for(AcademyPerson person : persons) {
			...
			// 해결법) 참조변수로 객체 형변환 가능한 클래스를 확인한 후 명시적 객체 형변환을 이용
			...
			// 오버라이드 선언된 메소드는 묵시적 객체 형변환에 의해 
			// 부모클래스의 메소드를 호출하지 않고 자식클래스의 메소드 호출
			// 참조변수에 저장된 자식클래스의 객체에 의해 자식클래스의 메소드가 선택 호출
			// => 오버라이드에 의한 다형성 : 객체 형변환에 의해 참조변수에 저장된 객체에 따라
			// 다른 자식클래스의 메소드를 선택 호출
			person.display();
			System.out.println("========================");
		}
