...
public class UpAndDownExample2 {
	public static void main(String[] args) {
		...
		for(int cnt=1;cnt<=10;cnt++) {
			...
			if(computer == user) {
				...
				System.out.println("[결과]"+user+"보다 큰 값을 입력해 보세요.");
			} else {
				System.out.println("[결과]"+user+"보다 작은 값을 입력해 보세요.");
			}
		}
		
		if(!result) {//정답을 맞추지 못한 경우
			System.out.println("[메세지]정답을 맞추지 못했군요. 정답은 ["+computer+"]입니다.");
		}
