...
// => 클라이언트와 연결된 Socket은 새로운 스레드를 생성하여 독립적으로 입출력되도록 설정
public class ChatServerApp {
	...
	// 현재 접송중인 모든 클라이언트에게 메세지를 전달하는 메소드
	public void sendMessage(String message) {
		...
		for (SocketThread socketThread : clientList) {
			...
			// => 외부클래스는 내부클래스로 객체를 생성하여 접근제한자에 상관없이
			// 필드와 메소드 접근 가능
	...
	public class SocketThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				// => autoFlush 매개변수에 [true]를 전달하면 버퍼를 사용하지 않고 무조건 출력스트림으로 데이터를 전달
