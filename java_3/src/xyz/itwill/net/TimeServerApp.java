...
public class TimeServerApp {
	public static void main(String[] args) {
		...
		try {
			...
			// ServerSocket.toString() : ServerSocket 객체에 저장된 접  속 관련 정보를
			// 문자열로 반환하는 메소드
			...
			while (true) {
				// ServerSocket.accept() : 클라이언트가 접속되면 클라이언트에 값을 주고 받을
				// 수 있는 Socket 객체를 반환하는 메소드
				// => 클라이언트가 접속되기 전까지 스레드 일지 중지되며 클라이언트가 접속되면
				...
				// 로그 처리 - 기록
				// Socket.getInetAddress() : 서버의 소켓과 연결된 외부 컴퓨터의 네트워크
				// 식별자가 저장된 InetAddress 객체를 반환하는 메소드
