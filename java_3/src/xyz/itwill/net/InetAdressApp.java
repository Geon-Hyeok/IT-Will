...
// TCP(Transmission Control Protocol) : 연결형 프로토콜(네트워크 기능을 이용한 연결 후 통신 가능)로
// 신뢰할 수 있는 데이터 전송에 대한 통신 규약 - 전송제어 프로토콜
// => 소켓(Socket) 통신 - 1:1 연결
// UDP(User Datagram Protocol) : 비연결형 프로토콜(네트워크 기능을 이용한 연결 설정 없이 통신가능)로
// 신뢰할 수 없는 데이터 전송에 통신규약 - 사용자 데이터그램 프로토콜
// => 고속통신 - 1:Many 연결
...
public class InetAdressApp {
	public static void main(String[] args) throws UnknownHostException {
		// InetAddress 클래스 : 네트워크 식별자(IP 주소 및 호스트 이름)를 저장하기 위한 클래스
		// InetAddress.getLocalHost() : 현재 사용중인 컴퓨터의 네트워크 식별자가 저장된
		// InetAdress 객체를 반환하는 메소드
		// => UnknownHostException 발생(일반예외) : 호스트이름의 컴퓨터를 검색할 수 없는 경우 발생되는 예외
		// 현재 사용중인 컴퓨터는 기본적으로 [127.0.0.1]의 IP주소가 제공되며 [localhost]라는 이름으로
		// 호스트이름 설정
