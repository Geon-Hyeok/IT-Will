...
// => 예외가 발생되어 프로그램에 강제로 종료되는 것을 방지하거나 
// 프로그램이 강제로 종료된 이유를 알기 위해 예외처리(ExceptionHandle) 필드
...
// => 자식클래스 : Exception 클래스, Error 클래스

// # 예외의 종류 
// 1. 일반예외 : 프로그램 실행시 예외가 발생될 가능성이 높아 예외처리를 하지 않은 경우 컴파일시 에러 발생
...
// 전달하여 참조변수에 저장 - 참조변수에 저장된 예외관련 객체의 메소드를 호출하여 예외처리
