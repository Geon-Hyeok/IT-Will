...
// 정규표현식(RegularExpression) : 메타문자(Meta Character), 회피문자(Escape Character)등을 사용하여
// 일정한 규칙의 문자열을 표현하는 방법 - 사용자 입력값에 대한 검증
/*
 ^문자 : 문자(열)로 시작됨을 의미
 문자$ : 문자(열)로 종료됨을 의미
 ...
 [문자1-문자2] : [문자]1에서 [문자2] 범위의 문자 중 하나를 의미
	...
	public static void main(String[] args) {
		...
		*/
		...
		if(!Pattern.matches(passwordReg, password)) {
			System.out.println("[에러] 형식에 맞는 비밀번호를 입력하세요");
			System.exit(0);
		}
		...
		*/
		...
		if(!Pattern.matches(emailreg, email)) {
			System.out.println("[에러] 형식에 맞는 이메일을 입력하세요");
			System.exit(0);
		}
