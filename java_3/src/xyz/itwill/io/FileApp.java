...
public class FileApp {
	public static void main(String[] args) throws IOException {
		...
		// 부모(디렉토리)와 자식(파일)을 전달받아 파일 경로가 저장된 File 객체 생성
		File fileThree = new File("c:/data", "itwill.txt");

		if (fileThree.exists()) {
			fileThree.delete();
			System.out.println("c:\\data\\itwill.txt 파일이 삭제되었습니다");
		} else {
		...
		// => 상대경로 표현방법은 [..] 기호를 사용하여 상위 디렉토리를 표현하며 [파일] 형식으로
		...
		if (fileFour.exists()) {
			...
			System.out.println("파일 경로 = " + fileFour); // toString 생략 가능
		...
		// false를 반환하고 디렉토리인경우 true를 반환하는 메소드
		if (fileFive.isDirectory()) {
			// File.listFiles() : File 객체에 저장된 파일경로의 디렉토리의 하위 파일이나
			// 하위 디렉토리 목록을 File 객체 배열로 반환하는 메소드
			File[] subFiles = fileFive.listFiles();
			
			System.out.println(fileFive + " 폴더의 자식 목록 >> ");
			for(File file : subFiles) {
				// File.isFile() : File 객체에 저장된 일경로의 파일이 일반파일이 아닌 경우
				// [false]를 반환하고 일반 파일인 경우 [true]를 반환
				if(file.isFile()) {
