...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			// System.out : Java 프로그램 개발을 위해 기본적으로 제공되는 모니터 출력 스트림이 저장된 필드
			// => OutputStream 클래스를 기반으로 제공되는 입력스트림(객체) - PrintStream 클래스
			// => 출력스트림에 값을 전달하여 모니터가 출력 처리
			// OutputStream.write(in b) : 매개변수로 제공받은 값을 원시데이터(1 Byte)로 출력스트림에 전달하는 메소드
